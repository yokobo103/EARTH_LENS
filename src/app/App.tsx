...
export function App() {
  ...
  const layerPanel = <LayerPanel lenses={displayLenses} activeLensIds={appMode === "explore" ? activeLensIds : missionLensIds} locale={locale} onToggle={appMode === "explore" ? toggleExploreLens : toggleMissionLens} missionRecommendedLensIds={appMode === "mission" ? currentMission.recommendedLensIds : undefined} bonusLens={appMode === "explore" ? { active: paleoToolEnabled, onToggle: () => { const nextEnabled = !paleoToolEnabled; setPaleoToolEnabled(nextEnabled); if (!nextEnabled && temporalSelection.mode === "deep-time") changeTime({ mode: "present", ageMa: 0 }); } } : undefined} />;
  ...
  return (
    <main lang={locale} className={`app-shell${locale === "ja" ? " ja-ui" : ""}${isCompact ? " compact-ui" : ""}${temporalSelection.mode === "deep-time" ? " deep-time-active" : ""}${appMode === "mission" ? " mission-mode" : ""}${showGlobe && appMode === "explore" && paleoToolEnabled ? " paleo-band-open" : ""}${missionView === "passport" && appMode === "mission" ? " passport-mode" : ""}${sheetOpen ? " anchor-sheet-open" : ""}${missionCleared ? " mission-cleared" : ""}`}>
      {showGlobe && <EarthGlobe activeLensIds={appMode === "explore" ? activeLensIds : missionLensIds} onFeatureSelect={handleGlobeFeature} onLocationSelect={handleGlobeLocation} temporalSelection={temporalSelection} appMode={appMode} missionEffects={missionEffects} missionFocus={missionFocus} ariaLabel={t(locale, "interactiveEarth")} locale={locale} selectedFeature={selectedFeature} anchorPoint={anchorPoint} anchorExpanded={anchorExpanded} anchorContent={appMode === "explore" ? (anchorPoint ? <AnchoredDetailsCard feature={selectedFeature} location={selectedLocation} anchorPoint={anchorPoint} expanded={anchorExpanded} whyHereResult={whyHereResult} isAnalyzing={isAnalyzing} locale={locale} onAnalyze={() => { setAnchorExpanded(true); void runWhyHere(); }} onClose={clearSelection} /> : null) : missionAnchorContent} onAnchorClose={clearSelection} initialCamera={sharedCamera} initialFeature={initialSharedView.feature ?? (!hasSharedView ? openingFeature : null)} terrainReliefEnabled={!vividEarth} onCameraChange={setSharedCamera} />}
