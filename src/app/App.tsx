...
export function App() {
  ...
  const clearSelection = () => { setSelectedFeature(null); setSharedFeature(null); setSelectedLocation(null); setAnchorPoint(null); setAnchorExpanded(false); setLensInfoOpen(false); setWhyHereResult(null); };
  ...
  // スマホでは展開カードを、地球に貼り付く吹き出しではなく下からのシートにする。
  // 開いている間はレンズの帯も凡例も畳む。地球の操作UIとシートを同じ指の下で争わせない。
  const sheetOpen = isCompact && showGlobe && Boolean(anchorPoint) && anchorExpanded;
  const missionCleared = appMode === "mission" && missionState.status === "completed";
  ...
  const layerPanel = <LayerPanel lenses={displayLenses} activeLensIds={appMode === "explore" ? activeLensIds : missionLensIds} locale={locale} onToggle={appMode === "explore" ? toggleExploreLens : toggleMissionLens} onInfoOpenChange={setLensInfoOpen} missionRecommendedLensIds={appMode === "mission" ? currentMission.recommendedLensIds : undefined} bonusLens={appMode === "explore" ? { active: paleoToolEnabled, onToggle: () => { const nextEnabled = !paleoToolEnabled; setPaleoToolEnabled(nextEnabled); if (!nextEnabled && temporalSelection.mode === "deep-time") changeTime({ mode: "present", ageMa: 0 }); } } : undefined} />;
  ...
  return (
    <main lang={locale} className={`app-shell${locale === "ja" ? " ja-ui" : ""}${isCompact ? " compact-ui" : ""}${temporalSelection.mode === "deep-time" ? " deep-time-active" : ""}${appMode === "mission" ? " mission-mode" : ""}${showGlobe && appMode === "explore" && paleoToolEnabled ? " paleo-band-open" : ""}${missionView === "passport" && appMode === "mission" ? " passport-mode" : ""}${sheetOpen ? " anchor-sheet-open" : ""}${lensInfoOpen ? " lens-info-open" : ""}${missionCleared ? " mission-cleared" : ""}`}>
      {showGlobe && <EarthGlobe activeLensIds={appMode === "explore" ? activeLensIds : missionLensIds} onFeatureSelect={handleGlobeFeature} onLocationSelect={handleGlobeLocation} temporalSelection={temporalSelection} appMode={appMode} missionEffects={missionEffects} missionFocus={missionFocus} ariaLabel={t(locale, "interactiveEarth")} locale={locale} selectedFeature={selectedFeature} anchorPoint={anchorPoint} anchorExpanded={anchorExpanded} lensInfoOpen={lensInfoOpen} anchorContent={appMode === "explore" ? (anchorPoint ? <AnchoredDetailsCard feature={selectedFeature} location={selectedLocation} anchorPoint={anchorPoint} expanded={anchorExpanded} whyHereResult={whyHereResult} isAnalyzing={isAnalyzing} locale={locale} onAnalyze={() => { setLensInfoOpen(false); setAnchorExpanded(true); void runWhyHere(); }} onClose={clearSelection} /> : null) : missionAnchorContent} onAnchorClose={clearSelection} initialCamera={sharedCamera} initialFeature={initialSharedView.feature ?? (!hasSharedView ? openingFeature : null)} terrainReliefEnabled={!vividEarth} onCameraChange={setSharedCamera} />}
