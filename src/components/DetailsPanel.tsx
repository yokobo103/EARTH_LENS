...
import type { EarthLensDefinition, GeographicPoint, LensFeature } from "../lenses/types";
...
export function DetailsPanel({ feature, location, analysisLocation = location, whyHereResult, isAnalyzing, locale, onAnalyze, embedded = false }: DetailsPanelProps) {
  ...
  return (
    <aside className={`glass-panel details-panel${embedded ? " is-embedded" : ""}${whyHereResult ? " has-scan-result" : ""}`} aria-label={whyHereResult ? t(locale, "scanResult") : "Observation information"}>
...
function FeatureDisclosure({ feature, locale }: { feature: LensFeature; locale: Locale }) {
  return <>
...
function FeatureDataDetails({ feature, lens, locale }: { feature: LensFeature; lens?: EarthLensDefinition; locale: Locale }) {
  const readings = feature.furtherReading ?? lens?.furtherReading;
  return <details className="observation-data-details">
    <summary><span>{t(locale, "dataDetails")}</span><b aria-hidden="true">+</b></summary>
    <div className="observation-data-details-body">
      ...
      {feature.provenance.derivedFrom && feature.provenance.derivedFrom.length > 0 && <section className="provenance-note"><strong>{t(locale, "derivedFrom")}</strong>{feature.provenance.derivedFrom.map((source) => <p key={source.source}>{source.source} · {source.note}</p>)}</section>}
      ...
      {feature.provenance.sourceUrl && <a className="source-link" href={feature.provenance.sourceUrl} target="_blank" rel="noreferrer">{t(locale, "viewReference")}</a>}
      {readings && readings.length > 0 && <section className="further-reading"><div className="further-reading-heading"><strong>{t(locale, "furtherReading")}</strong><small>{t(locale, "externalGuide")}</small></div>{readings.map((reading) => <article key={reading.url}><a href={reading.url} target="_blank" rel="noreferrer">{reading.title} ↗</a><p>{reading.note}</p><small>{reading.language === "en" ? t(locale, "englishOnly") : reading.language.toUpperCase()} · {t(locale, "checked")} {reading.checkedAt}</small></article>)}</section>}
    </div>
  </details>;
}

function formatCoordinates(location: GeographicPoint): string {
...
function whyHerePanelKey(result: WhyHereResult | null): string {
  if (!result) return "scan-idle";
  return [result.location.latitude, result.location.longitude, ...result.lensResults.map((lens) => `${lens.lensId}:${lens.nearbyCount}`)].join("|");
}
