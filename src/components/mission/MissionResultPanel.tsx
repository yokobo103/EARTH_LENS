...
interface MissionResultPanelProps { mission: EarthMission; state: MissionState; locale: Locale; onCollectSticker: () => void; whyHereResult?: WhyHereResult | null; isAnalyzing?: boolean; onAnalyze?: () => void; embedded?: boolean }

export function MissionResultPanel({ mission, state, locale, onCollectSticker, whyHereResult = null, isAnalyzing = false, onAnalyze = () => undefined, embedded = false }: MissionResultPanelProps) {
  return <aside className={embedded ? "mission-result-embedded" : "glass-panel mission-result-panel"} aria-label="Mission result">
