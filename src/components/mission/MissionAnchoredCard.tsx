...
export function MissionAnchoredCard({ mission, state, locale, expanded, whyHereResult, isAnalyzing, onSubmit, onAnalyze, onCollectSticker, onExpand, compact = false }: MissionAnchoredCardProps) {
  ...
  return <section ref={rootRef} className={`anchor-card-content mission-anchor-card${expanded ? " is-expanded" : ""}`} aria-label={title}>
    {state.status === "completed" ? <MissionResultPanel mission={mission} state={state} locale={locale} onCollectSticker={onCollectSticker} whyHereResult={whyHereResult} isAnalyzing={isAnalyzing} onAnalyze={onAnalyze} embedded compact={compact} /> : expanded ? <MissionObservationPanel state={state} locale={locale} onSubmit={onSubmit} embedded /> : <>
